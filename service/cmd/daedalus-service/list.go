// service.list 工具的实现面:过滤模式校验 + systemctl list-units 直发 +
// UNIT/LOAD/ACTIVE/SUB 前四列解析。工具注册仍在 main.go 的 newServer;本文件
// 与 main.go 同包,分担使两文件都稳居 250 纯 LOC 上限之下。
//
// 安全边界与 service.query 同源:过滤模式先经白名单字符类 + 遍历检查(argv
// 构造前 fail-closed 拒绝),再经 os/exec argv 直发,绝不经过 sh -c。
package main
...
// serviceListIn 对应 service.list(filter: str = "*")。Filter 用 *string 区分
// "客户端未提供"(nil → 默认值 "*")与"显式传空串"(→ 拒绝)。
type serviceListIn struct {
	Filter *string `json:"filter"`
}

// listFilterAll 是过滤模式的通配缺省值:filter 等于它时不追加过滤参数
// (systemctl 默认即列全部)。
const listFilterAll = "*"
...
// 单元名白名单字符类且不含 ".." 遍历的模式——glob 元字符、路径分隔符、空白、
// 非 ASCII、空字节天然被字符类拒掉,非法模式永不抵达 argv,也就没有 shell glob
// 展开或 systemctl 端语义逃逸的通道。filter 是模式而非单元名,故不做后缀补全。
...
// runSystemctlList argv 直发执行 `systemctl list-units ... [filter]`(绝不经过
// sh -c);filter 为 "*" 时省略过滤参数。结果按单元名显式升序排序——输出确定性
...
// parseListUnits 解析 list-units 的行输出:UNIT 列可能带 systemd 状态字形前缀
// (●/*/× 等),故不数固定列号,而是定位"首个匹配单元名字符类且以 .service
// 结尾的字段",字形前缀与对齐空格都被天然跳过;匹配不到单元名的行静默跳过。
// Properties 键与 service.query 白名单同名,DesiredState 恒空(观测态无期望)。
